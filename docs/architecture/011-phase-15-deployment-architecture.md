...
├── crawler      (NestJS worker — event discovery and AI extraction)
└── redis        (Railway managed Redis — BullMQ work queue)
```
...
## Railway Capability Boundary

Railway is the right platform through at least Milestone 2 and likely early Milestone 3. The table below records what it does and does not handle, so the platform choice can be revisited with specific evidence rather than speculation.

| Concern                                              | Railway | Notes                                                                                                                                                |
| ---------------------------------------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Next.js web app                                      | ✅      | Functional; not as purpose-built as Vercel for ISR/edge primitives                                                                                   |
| NestJS API                                           | ✅      | Standard Node.js process; no constraints                                                                                                             |
| PostgreSQL                                           | ✅      | Managed service with automated backups                                                                                                               |
| Worker services / cron jobs                          | ✅      | First-class; crawler and queue workers are just additional services                                                                                  |
| Redis / BullMQ queue                                 | ✅      | Railway-managed Redis; self-operated but low maintenance at this scale                                                                               |
| Playwright / headless Chromium                       | ✅      | Memory-intensive (~400–600 MB per instance); manageable at curated-source volume                                                                     |
| AI inference via API (Anthropic, OpenAI)             | ✅      | Outbound HTTP call from a worker; no platform constraints                                                                                            |
| Object / blob storage                                | ❌      | No S3 equivalent; use Cloudflare R2 or AWS S3 if large file storage is needed                                                                        |
| GPU compute / self-hosted AI models                  | ❌      | No GPU support; use Modal or RunPod if local model inference becomes necessary                                                                       |
| High-volume parallel crawling (thousands of sources) | ⚠️      | Railway can scale horizontally but burst compute is less turnkey than Lambda or Fargate; revisit at Milestone 3 if source volume grows significantly |

---

## Milestone Roadmap

Product milestone scope and sequencing are documented in `docs/specs/002-roadmap.md`. The deployment-relevant summary:

- **Milestone 1** — Three Railway services: web, API, database. No automated ingestion.
- **Milestone 2** — Single city launch. Adds crawler worker and Redis queue to the Railway project. Crawler feeds the existing submission/moderation workflow; no new database infrastructure required.
- **Milestone 3** — Geographical expansion. Same crawler pattern, more cities. Infrastructure limits should be revisited with usage evidence rather than in advance.

**Milestone 2 crawler stack (deployment view):**

- Crawler worker: NestJS worker service on Railway, nightly cron schedule
- Playwright handles JavaScript-rendered source pages
- Claude API call per candidate event: structured extraction + topic classification in one pass
- Raw HTML processed inline and discarded — no object storage required
- Extracted events written to PostgreSQL as `pending` submissions via the existing API
- Existing admin moderation queue handles editorial review before publication
