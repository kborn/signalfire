...
describe('AdminAuthController', () => {
  ...
  it('login creates a session cookie and returns ok', async () => {
    ...
    await expect(
      controller.login({ email: 'admin@example.com', password: 'FindYourFight1' }, response),
    ).resolves.toEqual({ ok: true });

    expect(serviceMock.login).toHaveBeenCalledWith('admin@example.com', 'FindYourFight1');
